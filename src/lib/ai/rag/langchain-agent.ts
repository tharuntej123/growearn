...
export class LangChainRAGService {
  ...
  static async executeRAG(
    ...
    try {
      const systemPrompt = `You are Growearn AI — the Senior Full-Stack Software Architect, Tech Career Advisor, and Community Guide for the Growearn platform.
...
3. If asked about people/mentors, courses, jobs, or features on Growearn ("are these guys available in the application growearn", "who can mentor me", "what courses are available"), reference the REAL context documents from the Growearn database. Confirm that verified mentors (like Sarah Jenkins, David Kim, Michael Chang, Priya Sharma, Marcus Vance, Elena Rostova) are active on Growearn and explain how users can explore their profiles or book 1-on-1 sessions.
...
CONTEXT DOCUMENTS (From Growearn Database & Platform):
${contextString}`;
